            ...
            if S[i]['E'] > 0:
                ...
                if S[i]['G'] <= 0:
                    ...
                    if(temp_rand <= (p / (1 - p * np.mod(r, np.round(1/p))))):
                        ...
                        S[i]['type'] = 'C'
                        S[i]['G'] = round(1/p) - 1
                        C = np.zeros(n+1, dtype=[('xd', float), ('yd', float), ('distance', float), ('id', int)])
                        C['xd'] = S[i]['xd']
                        C['yd'] = S[i]['yd']
                        distance = np.sqrt((S[i]['xd']-S[n]['xd'])**2 + (S[i]['yd']-S[n]['yd'])**2)
                        C['distance'] = distance
...
plt.figure(1)
plt.plot(r, STATISTICS['DEAD'][h, r+1])
plt.title('Dead Nodes')

# plt.figure(2)
# plt.plot(r, STATISTICS['ALLIVE'][h,r+1])
# plt.title('Live Nodes')

# plt.figure(3)
# plt.plot(r, STATISTICS['THROUGHPUT'][h,r+1])
# plt.title('THROUGHPUT')

plt.figure(4)
plt.plot(r, STATISTICS['ENERGY'][h,r+1])
